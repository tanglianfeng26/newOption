    ...
    <div class="mar_box">
      ...
      <div class="orderList">
        ...
        <div class="footerGoods">
          <div class="allPrice">
            共{{ goodsIndex }}件商品 商品金额：<span>￥{{ priceS }}</span>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  data() {
    return {
      title: "商品结算",
      rightIcon: "",
      datas: JSON.parse(this.$route.query.data),
      priceS: 0,
      numGoods: 0,
      arrAddress: [],
      getAddress: false,
  ...
  methods: {
    ...
    price() {
      ...
      return new Promise((resolve, reject) => {
        // this.priceS = this.datas.goodsIndex * Number(this.datas.price);
        this.datas.forEach((item, index) => {
          this.priceS += item.goodsIndex * Number(item.price);
          this.goodsIndex += item.goodsIndex;
        });
        resolve();
      });
    },
    ...
    submit_order() {
      var ObjGoods = JSON.parse(localStorage["shopCarList"]);
      this.datas.forEach(item => {
        ObjGoods = ObjGoods.filter((items, index) => {
          return items.code !== item.code;
        });
      });
      localStorage["shopCarList"] = JSON.stringify(ObjGoods);
    },
    chuan() {
      var self = this;
      Toast.success("下单成功");
